...
| handover_summary | 30 s; for shifts that ended (replay clock if replayed) | handover_summary |
| training_recommendations | daily 01:30 Asia/Kolkata | training_recommendations |
"""
...
async def handover_summary() -> None:
    await get_handover_job().tick()


async def training_recommendations() -> None:
    try:
        await run_in_threadpool(recommend_all, get_repo(), get_ai_repo(), get_telemetry())
    except Exception:  # noqa: BLE001 - try again tomorrow
        log.exception("training recommendations job failed")
...
def build_scheduler() -> AsyncIOScheduler:
    ...
    sched.add_job(
        handover_summary, "interval", seconds=HANDOVER_TICK_S, id="handover_summary", **common
    )
    sched.add_job(
