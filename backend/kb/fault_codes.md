...
1. Stay calm. Finish the current movement only if stopping mid-way is unsafe (for example, a load
   is lifted). Lower the bucket or attachment to the ground.
...
**What to do right now:**
1. This is the most urgent code. Low oil pressure can destroy the engine within minutes.
2. Lower the attachment to the ground and bring the machine to a safe stop on level ground.
3. **Shut down the engine as soon as it is safe to do so.** Do not keep working "to finish the task".
4. Do not restart the engine to "see if it goes away".
5. Once stopped and cool, check the oil level on the dipstick and look under the machine for oil.
...
**What to do right now:**
1. Lighten the work: take smaller bucket loads and stop holding the controls at the end of stroke.
...
Sometimes. A sensor glitch looks like **one reading jumping to an impossible value for about a
...
When in doubt, treat it as real and follow the safe steps.
