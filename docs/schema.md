...
- **Readable text IDs** for synthetic entities (`S1`, `M01`, `OP01`, `SH-2026-09-01-M01-D`) so CSVs
  from the generator load directly and demo screens are easy to read. Event tables use identity bigints.
- **Units in column names** (`coolant_temp_c`, `oil_pressure_kpa`) so nobody guesses.
- **`anomaly_label` / `anomaly_type` in `telemetry` are ground truth** from the generator.
...
- **RLS:** operators read shared fleet data and only their own personal rows (fatigue, training,
  chat, incidents); managers read everything. The backend uses the service role.

## Entity map
```mermaid
erDiagram
  sites ||--o{ machines : has
  sites ||--o{ operators : employs
  sites ||--o{ geofences : defines
  sites ||--o{ weather : records
  profiles }o--|| operators : "login for"
  shifts }o--|| operators : by
  shifts }o--|| machines : on
  shifts ||--o{ tasks : contains
  machines ||--o{ telemetry : streams
  machines ||--o{ machine_health_snapshots : scored
  machines ||--o{ maintenance_log : serviced
  machines ||--o{ maintenance_predictions : predicted
  machines ||--o{ alerts : raises
  alerts ||--o{ safety_events : links
  operators ||--o{ fatigue_log : monitored
  operators ||--o{ incidents : reports
  operators ||--o{ training_records : completes
  training_modules ||--o{ training_records : taken
  training_modules ||--o{ training_recommendations : suggested
  training_sessions ||--o{ session_bookings : booked
  documents ||--o{ document_chunks : split
```

## Tables at a glance

| Group | Table | Written by | Read by | Purpose |
|---|---|---|---|---|
| Reference | `sites` | generator | all | Construction sites |
| | `operators` | generator | all | Operator master data, skill score, hidden personality |
| | `machines` | generator, backend | all | Machine master data, current health score |
| | `profiles` | auth trigger | app | Login → role (operator/manager/admin) → operator_id |
| | `geofences` | manager | backend, app | No-go / pedestrian / speed zones as GeoJSON |
| Operations | `shifts` | generator, backend | app | Shift log, handover notes, LLM handover summary |
| | `weather` | generator | backend | Hourly weather per site |
| | `tasks` | generator, backend, operator | app, ML | Tasks, predictions (p10/p50/p90), actual duration |
| | `telemetry` | generator / replay | backend, ML | 1-minute machine signals |
| | `machine_health_snapshots` | backend | app | Digital twin subsystem scores |
| Safety | `alerts` | backend | app (realtime) | Every warning with graded-response stage |
| | `safety_events` | backend, vision, operator (SOS) | app, ML | Seatbelt, proximity, fatigue, tip, geofence, SOS |
| | `fatigue_log` | vision | backend | Per-minute fatigue signals |
| | `incidents` | operator, backend | app | Incident reports, offline-safe |
| Maintenance | `maintenance_log` | generator | ML | Services and failures (training labels) |
| | `maintenance_predictions` | backend | app | Failure probability per machine |
| Analytics | `fleet_metrics_weekly` | backend | manager | Weekly metrics, clusters, outliers, rank |
| Training | `training_modules` | seed | app | Library content and scenario definitions |
| | `training_records` | operator | app | Progress and scores |
| | `training_recommendations` | backend | app | Why a module was suggested |
| | `training_sessions`, `session_bookings` | seed, operator | app | Instructor booking (P2) |
| RAG | `documents`, `document_chunks` | ingest script | backend | Knowledge base with 384-d embeddings |
| | `chat_messages` | backend | app | Chat history with sources |
| MLOps | `model_runs` | training notebooks | manager | Model versions, params, metrics |
...
over WebSocket, not through Realtime (see `supabase.md`).
...
## Size budget (Supabase free tier = 500 MB)
Full 90-day telemetry is about 600k rows. Keep the full set as Parquet in `data/output/` for
training, and load only the **last 14 days** into Supabase for the app and replay.
